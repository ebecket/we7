...
<asp:Content ID="We7ContentPanel" ContentPlaceHolderID="MyContentPlaceHolder" runat="server">
  ...
  <script type="text/javascript" src="/Admin/Ajax/Ext2.0/adapter/ext/ext-base.js"></script>
    ...
    <h2 class="title" runat="server" id="TitleH2">
        ...
        <label id="NameLabel"><%=Message.PluginLabel %>管理</label>
        <span class="summary">
            <label id="SummaryLabel">通过安装<%=Message.PluginLabel %>，可能轻易扩展系统功能</label>
        </span>
        <div class="clear">
        </div>
    </h2>
    <div id="position"
        <a href="PluginList.aspx"><%=Message.PluginLabel %></a>&gt;<%=Message.PluginLabel %>管理
 <%--       <asp:Literal ID="PagePathLiteral" runat="server" Text="插件 >管理插件"></asp:Literal>--%>
    </div>
                    ...
                    <div>
                        <div id="plugin" class="toolbar2">
                            <li class="smallButton4">
                                <%--<asp:LinkButton ID="EnableLinkButton" runat="server" Text="启用" OnClick="EnableLinkButton_Click" />
                                <asp:LinkButton ID="DisableLinkButton" runat="server" Text="禁用" OnClick="DisableLinkButton_Click" />  --%>
                                <asp:LinkButton ID="UnstallLinkButton"  runat="server" 
                                Text="卸载" OnClientClick="return submitAction('uninstall');" /><a href="PluginAdd.aspx?pltype=<%=PluginType %>">
                                安装</a><asp:LinkButton ID="ctrDel"  runat="server" Text="删除"
                                 OnClientClick="return submitAction('delete');" />
                                <asp:LinkButton ID="DownLoadLinkButton"
                                 runat="server" Text="下载" onclick="DownLoadLinkButton_Click" />
                            </li>
                        </div>
                        <br />
                        <div style="min-height: 35px; width: 100%">
                            <asp:GridView ID="EnableGridView" runat="server" AutoGenerateColumns="false" CssClass="List"
                                GridLines="Horizontal" RowStyle-VerticalAlign="Middle" OnRowCommand="EnableGridView_RowCommand">
                                ...
                                <Columns>
                                    ...
                                    <asp:TemplateField HeaderText="缩略图" ItemStyle-Width="70px">
                                        <ItemTemplate>
                                            <img style="cursor: pointer;" src='<%# Eval("Directory","/Plugins/{0}/logo.jpg").ToString() %>'
                                    ...
                                    <asp:TemplateField HeaderText="操作">
                                        ...
                                        <% if (PluginType == PluginType.PLUGIN)
                                           { %><a href="#" onclick='return submitSingleAction("uninstall",<%# Eval("Directory","\"{0}\"") %>)'>
                                                卸载</a><%}
                                           else
                                           { %><a href="#" onclick='return submitSingleAction("delete",<%# Eval("Directory","\"{0}\"") %>)'>
                                                删除</a><%} %>|<% if (PluginType == PluginType.PLUGIN)
                                                                { %><a href="#" onclick='return submitSingleAction("<%#  Convert.ToBoolean(Eval("IsLocal"))?"updatelocal":"remoteupdate" %>",<%# Eval("Directory","\"{0}\"") %>)'>更新</a><%}
                                                                else
                                                                {%><a href="#" onclick='return submitSingleAction("<%#  Convert.ToBoolean(Eval("IsLocal")) ? "updatelocal" : "insctr" %>",<%# Eval("Directory","\"{0}\"") %>)'>更新</a>|<%} %><asp:LinkButton runat="server" Visible='<%# PluginType==PluginType.RESOURCE %>' ID="lnk" CommandArgument='<%# Eval("Directory") %>' CommandName="downloadzip">下载</asp:LinkButton>
                                        </ItemTemplate>
                                    </asp:TemplateField>
                                    <asp:TemplateField>
                                        <ItemTemplate>
                        ...
                        <div style="min-height: 35px; width: 100%">
                            <asp:GridView ID="DisableGridView" runat="server" AutoGenerateColumns="false" CssClass="List"
                                ...
                                <Columns>
                                    ...
                                    <asp:BoundField DataField="Name" HeaderText="名称" ItemStyle-Width="100px" />
                                    <asp:BoundField DataField="Version" HeaderText="版本" ItemStyle-Width="60px" />
                                    <%-- <asp:TemplateField HeaderText="状态" ItemStyle-Width="70px">
                                        <ItemTemplate>
                                            <asp:LinkButton ID="LinkButton3" runat="server" Text='未激活'></asp:LinkButton>
                                        </ItemTemplate>
                                    </asp:TemplateField>--%>
                                     <asp:TemplateField HeaderText="描述">
                                        <ItemStyle Width="500px" />
                                    ...
                                    <asp:TemplateField HeaderText="缩略图" ItemStyle-Width="70px">
                                        <ItemTemplate>
                                            <img style="cursor: pointer;" src='<%# Eval("Directory","/Plugins/{0}/").ToString()+Eval("Thumbnail") %>'
                                    ...
                                    <asp:TemplateField HeaderText="操作">
                                        <ItemStyle Width="100px" />
                                        <ItemTemplate>
                                            <a href="#" onclick='return submitSingleAction("install",<%# Eval("Directory","\"{0}\"") %>)'>
                                                激活</a>|<a href="#" onclick='return submitSingleAction("delete",<%# Eval("Directory","\"{0}\"") %>)'>删除</a>
                                        </ItemTemplate>
                                    </asp:TemplateField>
                                    <asp:TemplateField>
                                        <ItemTemplate>
