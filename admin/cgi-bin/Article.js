...
function articleOptionCheck(userCtrlID) {
	...
	//modify by qingspace
	var compareOwnerId="{d41dfc31-5e8e-4dc3-b8cb-7c4e03dc044a}";
	var channelddl=document.getElementById(userCtrlID + "_ChannelDropDownList");
	var authortxt=document.getElementById(userCtrlID + "_AuthorTextBox");
	var currentOwnerId="";
	if(channelddl)
	{
		var temp=channelddl.options[channelddl.selectedIndex].value;
		if(temp) currentOwnerId = selectItem;
	}else{
		var link=$("#position a[href*='oid=']").attr("href");
		var re=/{.*}/;
		currentOwnerId=(re.exec(link))[0];	
	}
	if(currentOwnerId && currentOwnerId == compareOwnerId)
	{
		if(item=="0" && authortxt.value == ''){
			alert("作者不能为空！");
            authortxt.focus();
            return false;			
		}		
